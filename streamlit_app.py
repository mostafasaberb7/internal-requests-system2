...
.role-card:hover { transform: translateY(-4px); box-shadow: 0 8px 30px rgba(0,0,0,0.15); }
.role-icon { font-size: 3rem; margin-bottom: 0.5rem; }
.role-title { font-size: 1.3rem; font-weight: 700; color: #1e3a5f; margin-bottom: 0.3rem; }
...
def request_card(row, card_cls="card-pending"):
    ...
    <div class="card {card_cls}">
      ...
      <p>🔍 <strong>المشرف:</strong> {supervisor}</p>
...
def page_select_role():
    ...
    <div class="app-header">
        ...
        <p>اختر دورك للمتابعة</p>
    </div>""", unsafe_allow_html=True)

    st.markdown('<div class="section-title">👋 مرحباً! من أنت؟</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="role-card">
            ...
            <div class="role-title">مندوب</div>
            <div class="role-desc">إرسال طلب جديد ومتابعة طلباتك</div>
        </div>""", unsafe_allow_html=True)
        if st.button("دخول كمندوب", key="btn_agent", type="primary"):
    ...
    with col2:
        ...
        <div class="role-card">
            <div class="role-icon">🔍</div>
            <div class="role-title">مشرف</div>
            <div class="role-desc">مراجعة الطلبات والموافقة أو الرفض</div>
        </div>""", unsafe_allow_html=True)
        if st.button("دخول كمشرف", key="btn_supervisor"):
    ...
    with col3:
        ...
        <div class="role-card">
            ...
            <div class="role-desc">الموافقة النهائية وعرض كل الطلبات</div>
        </div>""", unsafe_allow_html=True)
        if st.button("دخول كمدير", key="btn_manager"):
...
def page_agent():
    ...
    <div class="app-header">
        ...
        <p>إرسال ومتابعة الطلبات</p>
    </div>""", unsafe_allow_html=True)

    tab1, tab2 = st.tabs(["📝 إرسال طلب جديد", "📋 متابعة الطلبات"])
    ...
    with tab1:
        ...
        c1, c2 = st.columns(2)
        with c1:
            agent_name = st.text_input("اسم المندوب *", placeholder="أدخل اسمك الكامل")
        ...
        agent_email   = st.text_input("بريدك الإلكتروني (اختياري – لاستقبال الإشعارات)",
                                      placeholder="example@company.com")
        supervisor    = st.text_input("اسم المشرف المسؤول *", placeholder="أدخل اسم مشرفك")

        st.markdown('</div>', unsafe_allow_html=True)

        if st.button("✅ إرسال الطلب", type="primary"):
            if not all([agent_name.strip(), car_number.strip(), car_type.strip(),
                        problem_desc.strip(), supervisor.strip()]):
                ...
                st.markdown(
                    f'<div class="success-box">✅ تم إرسال طلبك بنجاح!<br>رقم الطلب: <strong>{req_id}</strong><br>احتفظ بهذا الرقم لمتابعة طلبك</div>',
                    unsafe_allow_html=True)
                st.balloons()

    with tab2:
        st.markdown('<div class="section-title">🔎 تتبع طلب برقمه</div>', unsafe_allow_html=True)
        track_id = st.text_input("أدخل رقم الطلب", placeholder="مثال: A1B2C3D4")
        if st.button("🔍 بحث", type="primary"):
            df = load_data()
            if not df.empty:
                result = df[df["ID"] == track_id.strip().upper()]
                if result.empty:
                    st.markdown('<div class="error-box">❌ لم يتم العثور على هذا الطلب</div>',
                                unsafe_allow_html=True)
                else:
                ...
                st.markdown('<div class="info-box">لا توجد طلبات مسجلة بعد</div>',
                            unsafe_allow_html=True)

        st.divider()
        st.markdown('<div class="section-title">📋 البحث باسم المندوب</div>', unsafe_allow_html=True)
        search_name = st.text_input("أدخل اسمك لعرض طلباتك", placeholder="اسمك الكامل")
        if st.button("🔍 عرض طلباتي"):
            df = load_data()
            if not df.empty:
                my_df = df[df["اسم المندوب"].str.strip() == search_name.strip()]
                if my_df.empty:
                    st.markdown('<div class="info-box">لا توجد طلبات بهذا الاسم</div>',
                                unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="info-box">عدد الطلبات: {len(my_df)}</div>',
                                unsafe_allow_html=True)
...
def page_supervisor():
    ...
    <div class="app-header">
        ...
        <p>مراجعة الطلبات المعلّقة</p>
    </div>""", unsafe_allow_html=True)

    # المشرف يكتب اسمه لعرض طلباته فقط
    sup_name = st.text_input("أدخل اسمك (المشرف) لعرض الطلبات الخاصة بك",
                             placeholder="اسمك كما كتبه المندوب")

    if not sup_name.strip():
        st.markdown('<div class="info-box">👆 أدخل اسمك أعلاه لعرض الطلبات</div>',
                    unsafe_allow_html=True)
        st.divider()
        if st.button("🔙 رجوع"):
    ...
    df = load_data()
    pending = df[
        (df["المشرف"].str.strip() == sup_name.strip()) & (df["الحالة"] == "Pending")
        ...
        for _, row in pending.iterrows():
            ...
            with c1:
                if st.button("✅ موافقة", key=f"sa_{req_id}", type="primary"):
                    ...
                    st.markdown('<div class="success-box">✅ تمت الموافقة</div>',
                                unsafe_allow_html=True)
                    st.rerun()
            with c2:
...
def page_manager():
    ...
    <div class="app-header">
        ...
        <p>الموافقة النهائية وعرض كل الطلبات</p>
    </div>""", unsafe_allow_html=True)
                ...
                with c1:
                    if st.button("✅ موافقة", key=f"ma_{req_id}", type="primary"):
                        ...
                        send_email(
                            ...
                            "تحديث حالة طلبك – نظام الطلبات الداخلية",
                            f"مرحباً {row['اسم المندوب']},\n\n"
                            f"تمت الموافقة النهائية على طلبك ✅\n"
                            f"• رقم الطلب: {req_id}\n• رقم السيارة: {row['رقم السيارة']}\n"
                            f"• نوع السيارة: {row['نوع السيارة']}\n• نوع الطلب: {row['نوع الطلب']}\n"
