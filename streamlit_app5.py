...
# ─── المستخدمون من Streamlit Secrets ───
# في secrets.toml بتعرّف كل يوزر هكذا:
# [users."أحمد علي"]
...
def request_card(row, card_cls="card-pending"):
    """يعرض الـ card بشكل صحيح مع الصيانة والتعليقات في markdown منفصل"""
    rid        = row.get("ID","")
    status     = row.get("الحالة","Pending")
    agent      = row.get("اسم المندوب","")
    car_num    = row.get("رقم السيارة","")
    car_type   = row.get("نوع السيارة","")
    req_type   = row.get("نوع الطلب","")
    problem    = row.get("وصف المشكلة","")
    sup        = row.get("المشرف","")
    ts         = row.get("Timestamp","")
    maint_note = row.get("ملاحظات الصيانة","")
    last_maint = row.get("آخر تاريخ صيانة","")
    sup_com    = row.get("تعليق المشرف","")
    mgr_com    = row.get("تعليق المدير","")

    # الكارد الأساسية
    st.markdown(
        f'''<div class="card {card_cls}">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
        <strong>🔖 #{rid}</strong>{status_badge(status)}
      </div><hr style="margin:6px 0;">
      <p>👤 <strong>المندوب:</strong> {agent}</p>
      <p>🚗 <strong>السيارة:</strong> {car_num} – {car_type}</p>
      <p>📌 <strong>نوع الطلب:</strong> {req_type}</p>
      <p>📝 <strong>التفاصيل:</strong> {problem}</p>
      <p>🔍 <strong>المشرف:</strong> {sup}</p>
      <p>🕐 <strong>التاريخ:</strong> {ts}</p>
    </div>''', unsafe_allow_html=True)

    # ملاحظات الصيانة – منفصلة
    if maint_note or last_maint:
        st.markdown(
            f'''<div style="background:#f5f3ff;border-radius:8px;padding:0.6rem 1rem;
                        margin:-0.5rem 0 0.5rem 0;border-right:4px solid #7c3aed;">
              <p style="margin:4px 0;">🔧 <strong>ملاحظات الصيانة:</strong> {maint_note}</p>
              <p style="margin:4px 0;">📅 <strong>آخر تاريخ صيانة:</strong> {last_maint}</p>
            </div>''', unsafe_allow_html=True)

    if sup_com:
        st.markdown(
            f'''<div style="background:#f0fdf4;border-radius:8px;padding:0.5rem 1rem;
                        margin:2px 0;border-right:4px solid #10b981;">
              <p style="margin:2px 0;">💬 <strong>تعليق المشرف:</strong> {sup_com}</p>
            </div>''', unsafe_allow_html=True)

    if mgr_com:
        st.markdown(
            f'''<div style="background:#eff6ff;border-radius:8px;padding:0.5rem 1rem;
                        margin:2px 0;border-right:4px solid #2d6a9f;">
              <p style="margin:2px 0;">💬 <strong>تعليق المدير:</strong> {mgr_com}</p>
            </div>''', unsafe_allow_html=True)
...
def page_maintenance():
    ...
    <div class="app-header">
        <h1>🔧 مرحباً، {info['name']}</h1>
        <p>لوحة تحكم مشرف الصيانة – صلاحية تعديل كاملة على كل الطلبات</p>
    </div>""", unsafe_allow_html=True)
    show_user_bar()

    st.markdown("""
    <div class="purple-box">
        🔑 صلاحيتك: <strong>تعديل كامل</strong> على جميع بيانات الطلب +
        عمودي <strong>ملاحظات الصيانة</strong> و<strong>آخر تاريخ صيانة</strong>
        اللي هما مقفولين على باقي المستخدمين
    </div>""", unsafe_allow_html=True)
    ...
    # ── إحصائيات ──
    new_count  = len(df[df["الحالة"] == "Pending"])
    done_count = len(df[df["الحالة"] != "Pending"])
    c1, c2, c3 = st.columns(3)
    c1.metric("📋 إجمالي الطلبات", len(df))
    c2.metric("🆕 تنتظر مراجعة الصيانة", new_count)
    c3.metric("✅ تمت مراجعتها", done_count)
    st.divider()
    ...
    for _, row in df.iterrows():
        ...
        cls = "card-approved" if "Approved" in status else \
              "card-rejected" if "Rejected" in status else \
              "card-waiting"  if "Maintenance" in status else "card-pending"

        # عرض الكارد الحالية
        request_card(row, cls)

        # ── فورم التعديل الكامل ──
        expanded = (status == "Pending")  # افتح تلقائي للطلبات الجديدة
        with st.expander(f"✏️ تعديل الطلب #{req_id}", expanded=expanded):

            st.markdown('<div class="section-title">📋 بيانات الطلب</div>',
                        unsafe_allow_html=True)
            ec1, ec2 = st.columns(2)
            with ec1:
                e_agent = st.selectbox(
                    "👤 اسم المندوب",
                    AGENT_NAMES,
                    index=AGENT_NAMES.index(row.get("اسم المندوب","")) 
                          if row.get("اسم المندوب","") in AGENT_NAMES else 0,
                    key=f"e_agent_{req_id}")
                e_car_num = st.selectbox(
                    "🚗 رقم السيارة",
                    CAR_NUMBERS,
                    index=CAR_NUMBERS.index(row.get("رقم السيارة",""))
                          if row.get("رقم السيارة","") in CAR_NUMBERS else 0,
                    key=f"e_carnum_{req_id}")
            with ec2:
                e_car_type = st.text_input(
                    "🏷️ نوع السيارة",
                    value=row.get("نوع السيارة",""),
                    key=f"e_cartype_{req_id}")
                e_req_type = st.selectbox(
                    "📌 نوع الطلب",
                    REQUEST_TYPES,
                    index=REQUEST_TYPES.index(row.get("نوع الطلب",""))
                          if row.get("نوع الطلب","") in REQUEST_TYPES else 0,
                    key=f"e_reqtype_{req_id}")

            e_problem = st.text_area(
                "📝 وصف المشكلة",
                value=row.get("وصف المشكلة",""),
                key=f"e_prob_{req_id}",
                height=100)

            e_sup = st.selectbox(
                "🔍 المشرف المسؤول",
                SUPERVISOR_NAMES,
                index=SUPERVISOR_NAMES.index(row.get("المشرف",""))
                      if row.get("المشرف","") in SUPERVISOR_NAMES else 0,
                key=f"e_sup_{req_id}")

            st.markdown(
                '''<div style="background:#f5f3ff;border-radius:8px;padding:0.8rem 1rem;
                            margin:0.8rem 0;border-right:4px solid #7c3aed;">
                  <strong>🔒 حقول حصرية لمشرف الصيانة</strong>
                </div>''', unsafe_allow_html=True)

            e_note = st.text_area(
                "🔧 ملاحظات الصيانة",
                value=maint_note,
                key=f"e_note_{req_id}",
                placeholder="اكتب ملاحظاتك عن حالة السيارة والمشكلة...")

            default_date = datetime.today()
            if last_maint:
                try:
                    default_date = datetime.strptime(last_maint[:10], "%Y-%m-%d")
                except Exception:
                    default_date = datetime.today()

            e_date = st.date_input(
                "📅 آخر تاريخ إصلاح / شراء",
                value=default_date,
                key=f"e_date_{req_id}",
                format="YYYY/MM/DD")

            # ── زر الحفظ ──
            if st.button(f"💾 حفظ كل التعديلات – #{req_id}",
                         key=f"msave_{req_id}", type="primary"):
                if not e_note.strip():
                    st.markdown(
                        '<div class="error-box">⚠️ ملاحظات الصيانة مطلوبة قبل الحفظ</div>',
                        unsafe_allow_html=True)
                else:
                    new_status = "Reviewed by Maintenance" if status == "Pending" else status
                    update_row(req_id, {
                        "اسم المندوب":       e_agent,
                        "رقم السيارة":       e_car_num,
                        "نوع السيارة":       e_car_type,
                        "نوع الطلب":         e_req_type,
                        "وصف المشكلة":       e_problem,
                        "المشرف":            e_sup,
                        "ملاحظات الصيانة":   e_note,
                        "آخر تاريخ صيانة":  str(e_date),
                        "الحالة":            new_status,
                    })
                    st.markdown(
                        '<div class="success-box">✅ تم حفظ جميع التعديلات بنجاح</div>',
                        unsafe_allow_html=True)
                    st.rerun()
...
def page_supervisor():
    ...
    df       = load_data()
    all_mine = df[df["المشرف"] == sup_name].copy() if not df.empty else pd.DataFrame()
    ready    = all_mine[all_mine["الحالة"] == "Reviewed by Maintenance"]                if not all_mine.empty else pd.DataFrame()

    # إحصائيات سريعة
    total    = len(all_mine)
    n_ready  = len(ready)
    approved = len(all_mine[all_mine["الحالة"].str.contains("Approved", na=False)])
    rejected = len(all_mine[all_mine["الحالة"].str.contains("Rejected", na=False)])

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("📋 إجمالي طلباتي", total)
    c2.metric("⏳ تنتظر قراري",   n_ready)
    c3.metric("✅ معتمدة",         approved)
    c4.metric("❌ مرفوضة",         rejected)

    tab1, tab2 = st.tabs(["📬 طلبات تنتظر قراري", "📂 كل طلباتي"])

    # ── تبويب ١: الطلبات الجاهزة للمراجعة ──
    with tab1:
        if ready.empty:
            st.markdown(
                '<div class="warn-box">⏳ لا توجد طلبات جاهزة للمراجعة،'
                '<br>انتظر حتى تنهي الصيانة مراجعتها</div>',
                unsafe_allow_html=True)
        else:
            st.markdown(
                f'<div class="info-box">📬 جاهزة للمراجعة: <strong>{n_ready}</strong></div>',
                unsafe_allow_html=True)
            for _, row in ready.iterrows():
                req_id = row.get("ID", "")
                request_card(row, "card-waiting")
    ...
    # ── تبويب ٢: كل الطلبات (جدول + تحميل Excel) ──
    with tab2:
        if all_mine.empty:
            st.markdown('<div class="info-box">📭 لا توجد طلبات مرتبطة بك بعد</div>',
                        unsafe_allow_html=True)
        else:
            # فلتر الحالة
            status_options = ["الكل"] + [STATUS_AR[s] for s in STATUS_AR if s in all_mine["الحالة"].values]
            sel = st.selectbox("🔎 فلتر حسب الحالة", status_options, key="sup_filter")

            show = all_mine.copy()
            if sel != "الكل":
                rev = {v: k for k, v in STATUS_AR.items()}
                show = show[show["الحالة"] == rev.get(sel, sel)]

            st.markdown(f'<div class="info-box">عدد الطلبات: <strong>{len(show)}</strong></div>',
                        unsafe_allow_html=True)

            # عرض الكاردات
            for _, row in show.iterrows():
                s   = row.get("الحالة", "Pending")
                cls = "card-approved" if "Approved" in s else                       "card-rejected" if "Rejected" in s else                       "card-waiting"  if "Maintenance" in s else "card-pending"
                request_card(row, cls)
                st.divider()

            # جدول ملخص
            st.markdown('<div class="section-title">📊 جدول ملخص</div>',
                        unsafe_allow_html=True)
            disp = show.copy()
            disp["الحالة"] = disp["الحالة"].map(lambda x: STATUS_AR.get(x, x))
            cols = ["ID","اسم المندوب","رقم السيارة","نوع الطلب",
                    "الحالة","آخر تاريخ صيانة","Timestamp"]
            st.dataframe(disp[[c for c in cols if c in disp.columns]],
                         use_container_width=True)

            # تحميل Excel
            if os.path.exists(EXCEL_FILE):
                # نصدّر فقط طلبات المشرف الحالي
                import io
                out = io.BytesIO()
                all_mine.to_excel(out, index=False)
                out.seek(0)
                st.download_button(
                    "📥 تحميل طلباتي (Excel)",
                    data=out,
                    file_name=f"طلبات_{sup_name}_{datetime.now().strftime('%Y%m%d')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
